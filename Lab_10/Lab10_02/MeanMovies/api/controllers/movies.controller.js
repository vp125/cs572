...
module.exports.searchMovies= function(req,res) {
    console.log("SEARCH movies");
    const searchMovie = {};
    if(req.query.title){
        searchMovie.title = req.query.title;
    }
    if(req.query.year) {
        searchMovie.year = req.query.year;
    }
    if(req.query.rate) {
        searchMovie.rating = req.query.rate;
    }
    Movie.find(searchMovie).exec(function(err,foundMovies) {
        const response = {
            status: 200,
            message: foundMovies
        }
